...
export default function ProductsPage() {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (activeTab === 'low-stock')
      return matchesSearch && product.stock < LOW_STOCK_THRESHOLD;
  ...
  const handleDelete = async (product: Product) => {
    if (!confirm(t('deleteConfirm.description', { name: product.name })))
      return;
    try {
        ...
        {totalPages > 1 && (
          <section className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              {t('pagination.showing')} {(currentPage - 1) * ITEMS_PER_PAGE + 1}{' '}
              {t('pagination.to')}{' '}
              {Math.min(currentPage * ITEMS_PER_PAGE, filteredProducts.length)}{' '}
              {t('pagination.of')} {filteredProducts.length}{' '}
